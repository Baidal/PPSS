...
public class ClienteDAO_IT {
    ...
    public void testInsert() throws Exception {
        Cliente cliente = new Cliente(1,"John", "Domínguez");
    ...
    public void testUpdate() throws Exception {
        ...
        //Inicializamos la base de datos con el cliente a modificar
        IDataSet dataSet = new FlatXmlDataFileLoader().load("/cliente-esperado.xml");
        databaseTester.setDataSet(dataSet);
        databaseTester.onSetup();

        //Modificamos el usuario desde clienteDAO
        Assertions.assertDoesNotThrow(()->clienteDAO.update(cliente));

        //Recuperamos la tabla que hemos modificado
        IDataSet databaseDataset = connection.createDataSet();
        ITable actualTable = databaseDataset.getTable("cliente");

        //Cargamos la tabla que debería haber resultado de modificar el usuario
        IDataSet expectedDataset = new FlatXmlDataFileLoader().load("/cliente-modificado.xml");
        ITable expectedTable = expectedDataset.getTable("cliente");
    ...
    public void testRetrieve() throws Exception {
        ...
        //Inicializamos la base de datos con el cliente a retrivear
        IDataSet dataSet = new FlatXmlDataFileLoader().load("/cliente-esperado.xml");
        databaseTester.setDataSet(dataSet);
        databaseTester.onSetup();

        //Hacemos retrieve del cliente (cliente obenido)
        Cliente actualCliente = Assertions.assertDoesNotThrow(()->clienteDAO.retrieve(1));

        //Cargamos en la tabla el cliente esperado
        IDataSet expectedDataset = new FlatXmlDataFileLoader().load("/cliente-esperado.xml");
        ITable expectedTable = expectedDataset.getTable("cliente");

        //Creamos el cliente a partir del dataset esperado
        Cliente expectedCliente = new Cliente(Integer.parseInt((String) expectedTable.getValue(0,"id")),
                (String) expectedTable.getValue(0,"nombre"),
                (String) expectedTable.getValue(0,"apellido"));
        expectedCliente.setDireccion((String) expectedTable.getValue(0,"direccion"));
        expectedCliente.setCiudad((String) expectedTable.getValue(0,"ciudad"));

        Assertions.assertEquals(expectedCliente,actualCliente);
